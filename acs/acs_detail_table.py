...
def collect_subject_table(subject: str, year: str, city_geoid: str):
    ...
    """
    temp = base_url_subject(year) + f"?get=group({subject})&for=place:{city}&in=state:{STATE}&key={API_KEY}"
...
def year_df_subject(subject: str, year: str, city: str = PALM_SPRINGS):
    """
    Placeholder for now. We need a cleaned series IDs dataset for subject tables before we continue :)
    """
    return None
